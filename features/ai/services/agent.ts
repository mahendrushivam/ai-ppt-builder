...
import { AiErrorCode, AiPhase, type AiStreamEvent, AiStreamEventType, type ChatHistoryMessage } from "../types";
import { serializeDeckForModel } from "../utils/deck-context";
import { type AssembledToolCall, mergeToolCallDeltas, type SarvamChunk } from "../utils/sarvam-stream";
import { SarvamError, SarvamErrorKind, type SarvamMessage, streamCompletion } from "./sarvam-client";
import { CHAT_TOOLS, ChatToolName, executeToolCall } from "./tools";
...
type RoundResponse = { content: string; toolCalls: AssembledToolCall[]; finishReason: string | null };
 ...
 */
async function readRound(chunks: AsyncIterable<SarvamChunk>, deck: Deck, emit: Emit): Promise<RoundResponse> {
  const response: RoundResponse = { content: "", toolCalls: [], finishReason: null };
...
function errorEvent(error: unknown): AiStreamEvent {
